/*
growing_arrays.c - make growing arrays

input: function calls
output: integers depending o nwhat function was called
...
int add_record(Record new_record) {
    if (table.nval == table.max) {  // checks if array needs to be resized
        table.max *= GROW_SIZE;
        table.record = realloc(table.record, sizeof(Record) * table.max);
    }
   
    table.record[table.nval++] = new_record;  // add new record
    return (table.nval - 1);
}

// return 0 if there is no matching record
// return 1 if there is a matching record and deletes it and moves records
int del_record(const char *name) {
    for (int i = 0; i < table.nval; i++) {
        if (strcmp(table.record[i].name, name) == 0) {
            memmove(&table.record[i], &table.record[i+1], (table.nval-i-1) * sizeof(Record)); // move records after deleted one
            table.nval--;
            return 1;
        }
    }
...
Record get_record(int index) {
    return table.record[index]; // returns the record at the specified index
}
